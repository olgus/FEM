...
int Mesh_1D_L1::numerate_functions ()
{
	int functions[2];
...
void Mesh_1D_L1::input_mesh_data (char * file_name)
{
...
bool Mesh_1D_L1::make_init_Mesh ()
{
	Node_1D node;
	...
	for (int i = 0; i < n_axis[0]; i++)
	{
		...
		for (int j = 0; j < n_sections[i]; j++)
		{
			...
			// add new element
			std::unique_ptr <Element_1D_L1> element = std::make_unique<Element_1D_L1> ();
...
bool Mesh_1D_L1::need_to_numerate_edges ()
{
	return false;
}

bool Mesh_1D_L1::build_Mesh (std::vector<double> coordinates)
{
...
void Mesh_1D_L1::reset_elements ()
{
	// clear all possible old elements
	elements.clear ();
...
Mesh_1D_L1::Mesh_1D_L1 (const Mesh_1D_L1 & mesh)
{
	...
	for (const auto& e : mesh.elements)
		elements.push_back (std::make_unique<Element_1D_L1> (*e));
}
