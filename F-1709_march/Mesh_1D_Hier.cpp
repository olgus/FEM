...
int Mesh_1D_Hier::numerate_functions ()
{
	int functions[3];
...
void Mesh_1D_Hier::input_mesh_data (char * file_name)
{
...
bool Mesh_1D_Hier::make_init_Mesh ()
{
	Node_1D node;
	...
	for (int i = 0; i < n_axis[0]; i++)
	{
		...
		for (int j = 0; j < n_sections[i]; j++)
		{
			...
			// add new element
			std::unique_ptr <Element_1D_hier> element = std::make_unique<Element_1D_hier> ();
...
bool Mesh_1D_Hier::need_to_numerate_edges ()
{
	return false;
}

bool Mesh_1D_Hier::build_Mesh (std::vector<double> coordinates)
{
...
Mesh_1D_Hier::Mesh_1D_Hier (const Mesh_1D_Hier & mesh)
{
	...
	for (const auto& e : mesh.elements)
		elements.push_back (std::make_unique<Element_1D_hier> (*e));
}
